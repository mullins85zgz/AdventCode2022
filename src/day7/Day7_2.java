...
public class Day7_2 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            pintaEstructura(0,raizStatic);  //Mantenemos que es el que informa los campos
//            System.out.println(sumaCantidad(0,raizStatic));

            int totalDisk = 70000000;
            int required = 30000000;
            int used = raizStatic.getTotalSize();
            int available = totalDisk - used;
            int minimunSize = required - available;
            System.out.println("Disponible ");


            int respuesta =  buscaDirABorrar(raizStatic,minimunSize, raizStatic.getTotalSize());
    ...
    public static int buscaDirABorrar(MiDirectorio2 raiz,int size,int sizeAux){
        int smallest = sizeAux;
        if(raiz.getTotalSize()>size && smallest>raiz.getTotalSize())
            smallest = raiz.getTotalSize();
        for (MiDirectorio2 dir:raiz.getDirs().values()){
            smallest=buscaDirABorrar(dir,size,smallest);
        }
...
class MiFichero2 {
    ...
    public MiFichero2(String fileName, int size) {
        this.fileName = fileName;
        this.size = size;
    }
}
...
class MiDirectorio2 {
    ...
    public MiDirectorio2(String folderName) {
        this.folderName = folderName;
    }

    public MiDirectorio2() {
    }
